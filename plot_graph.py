...
while i < len(lines):
    if lines[i].startswith("["):
        ...
        # Parse memory stats from next lines if available
        if i + 3 < len(lines):
            vm_size_val = int(lines[i+1].split()[1]) if len(lines[i+1].split()) > 1 else 0
            vm_rss_val = int(lines[i+2].split()[1]) if len(lines[i+2].split()) > 1 else 0
            vm_swap_val = int(lines[i+3].split()[1]) if len(lines[i+3].split()) > 1 else 0
        else:
            vm_size_val = vm_rss_val = vm_swap_val = 0

        vm_size.append(vm_size_val)
        vm_rss.append(vm_rss_val)
        vm_swap.append(vm_swap_val)

        i += 5  # Skip to next record
    else:
        i += 1
...
plt.tight_layout()
plt.savefig("static/graph.png")  # Save inside static folder
plt.close()